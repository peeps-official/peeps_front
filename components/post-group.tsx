...
const PostGroup: NextPage<PostGroupType> = ({
  ...
  let profile = "absolute h-[42.86%] w-[5.09%] top-[28.57%] right-[92.03%] bottom-[28.57%] left-[2.89%] rounded-[50%] max-w-full overflow-hidden max-h-full object-cover z-[1]";
    ...
    >
      <div className="self-stretch h-[70px] relative">
        ...
        <div className="flex flex-center absolute h-[57.14%] w-[86.44%] top-[21.43%] right-[2.89%] bottom-[21.43%] left-[10.67%] rounded-8xs bg-whitesmoke-300 z-[1]">
          <input type='text' placeholder='...' className="pl-3 w-100 outline-none bg-transparent"
...
const OnePostFeed: NextPage<PostGroupType> = ({propPadding, propLeft2,propWidth1}) => {
  let prop = "/myprofile.png";
  const frameDivStyle: CSSProperties = useMemo(() => {
    return {
      padding: propPadding,
  ...
  return (
    <div className="self-stretch rounded-lg bg-white shadow-[0px_1px_1px_rgba(0,_0,_0,_0.1),_0px_0px_1px_rgba(0,_0,_0,_0.25)] overflow-hidden flex flex-col items-center justify-start py-[15px] pr-[13px] pl-3.5 box-border relative gap-[7px_0px] max-w-full">
        <div className="self-stretch flex flex-row items-start justify-between pt-0 pb-px pr-0.5 pl-0 gap-[20px] text-xs text-gray-100">
          ...
          {/* 게시글 더보기 ... */}
          <img
            className="h-6 w-[24.1px] relative overflow-hidden shrink-0 object-cover cursor-pointer"
